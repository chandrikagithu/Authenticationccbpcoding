...
const uri = `mongodb+srv://${process.env.DB_USER_ID}:${process.env.DB_PASSWORD}@cluster0.m4rht.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const subjectStudentsMapping = database.collection(
      "subjectStudentsMapping"
    );
    ...
    app.get("/getSubjects", async (req, res) => {
      ...
      if (findStudent) {
        const cursor = await subjectStudentsMapping.find({
          customerId: findStudent.customerId,
        });
